            ...
            for (let i = 0; i < changes.length; i++) if (!syncData.failedItems.includes(changes[i].itemId)) {
                ...
                switch (changes[i].status) {
                    ...
                    case "added_by_user":
                        ...
                        if (item) {
                            ...
                            if (syncData.type == "Contacts" && item.isMailList) {
                                ...
                                // Send master series cleanly (Exceptions block skipped for EAS 16.1)
                                ...
                                // ──────────────────────────────────────────────────────────────
                                // EAS 16.1 LONG-TERM FIX: Handle exceptions that already exist at creation time
                                // ──────────────────────────────────────────────────────────────
                                if ((asversion === "16.0" || asversion === "16.1") &&
                                    syncData.type === "Calendar" &&
                                    item.recurrenceInfo) {

                                    let exceptionIds = item.recurrenceInfo.getExceptionIds({});
                                    for (let excId of exceptionIds) {
                                        let exceptionItem = item.recurrenceInfo.getExceptionFor(excId);
                                        let instanceId = eas.tools.getIsoUtcString(excId);

                                        wbxml.otag("Change");
                                        wbxml.atag("ServerId", changes[i].itemId);   // master ServerId
                                        wbxml.otag("ApplicationData");
                                        wbxml.switchpage(syncData.type);
                                        wbxml.append(await (eas.sync.Calendar || {}).getWbxmlFromThunderbirdException(exceptionItem, syncData, instanceId));
                                        wbxml.switchpage("AirSync");
                                        wbxml.ctag();
                                        wbxml.ctag();
                                        c++;
                    ...
                    case "modified_by_user":
                        ...
                        if (item) {
                            ...
                            if (syncData.type == eas.sync.getEasItemType(item)) {
                                ...
                                if (isException && (asversion === "16.0" || asversion === "16.1")) {
                                    // EAS 16.1 exception modify → use InstanceId on the master
                                    let instanceId = eas.tools.getIsoUtcString(item.recurrenceId);
                                    wbxml.otag("Change");
                                    wbxml.atag("ServerId", changes[i].itemId);   // <-- this must be the MASTER ServerId, not the exception's ID!
                                    wbxml.otag("ApplicationData");
                                    wbxml.switchpage(syncData.type);
                                    wbxml.append(await (eas.sync.Calendar || {}).getWbxmlFromThunderbirdException(item, syncData, instanceId));
                                    wbxml.switchpage("AirSync");
                                    wbxml.ctag();
                                    wbxml.ctag();
                                    changedItems.push(changes[i].itemId);   // we still track the master
                                    sendItems.push({ type: changes[i].status, id: changes[i].itemId });
                                    c++;
                                } else {
                                    // normal master change or old EAS version
                                    wbxml.otag("Change");
                    ...
                    case "deleted_by_user":
                        let asversion = syncData.accountData.getAccountProperty("asversion");
                        let itemForDelete = await syncData.target.getItem(changes[i].itemId);

                        if (asversion === "16.0" || asversion === "16.1") {
                            if (itemForDelete && (eas.sync.Calendar || {}).isExceptionItem(itemForDelete)) {
                                // EAS 16.1 exception delete → Change with InstanceId + Deleted=1
                                let instanceId = eas.tools.getIsoUtcString(itemForDelete.recurrenceId);
                                wbxml.otag("Change");
                                wbxml.atag("ServerId", changes[i].itemId);   // master ServerId
                                wbxml.otag("ApplicationData");
                                wbxml.switchpage(syncData.type);
                                ...
                                // normal delete (master or old version)
                                wbxml.otag("Delete");
                                wbxml.atag("ServerId", changes[i].itemId);
                                wbxml.ctag();
                            }
        ...
        if (item.recurrenceInfo && (syncData.type == "Calendar" || syncData.type == "Tasks")) {
            ...
            for (let recRule of item.recurrenceInfo.getRecurrenceItems({})) {
                ...
                if (recRule.type == "WEEKLY") {
                    type = 1;
                    if (!weekDays.length) weekDays = [startDate.weekday + 1];
                } else if (recRule.type == "MONTHLY" && weeks.length) {
                    type = 3;
                } else if (recRule.type == "MONTHLY") {
                    type = 2;
                    if (!monthDays.length) monthDays = [startDate.day];
                } else if (recRule.type == "YEARLY" && weeks.length) {
                    type = 6;
                } else if (recRule.type == "YEARLY") {
                    type = 5;
                    if (!monthDays.length) monthDays = [startDate.day];
                    if (!months.length) months = [startDate.month + 1];
                }
                wbxml.atag("Type", type.toString());

                // Tasks need a Start tag
                if (localStartDate) wbxml.atag("Start", localStartDate);

                // DayOfMonth
                if (monthDays[0]) wbxml.atag("DayOfMonth", monthDays[0].toString());

                // DayOfWeek
                if (weekDays.length) {
                    let bitfield = 0;
                    for (let day of weekDays) bitfield |= 1 << (day - 1);
                ...
                // MonthOfYear
                if (months.length) wbxml.atag("MonthOfYear", months[0].toString());
                ...
                // WeekOfMonth
                if (weeks.length) wbxml.atag("WeekOfMonth", weeks[0].toString());

                wbxml.ctag();
            }

            // ──────────────────────────────────────────────────────────────
            // EAS 16.1: NO <Exceptions> block in master item any more
            // Exceptions are sent as separate Change + InstanceId commands
            // ──────────────────────────────────────────────────────────────
            if (asversion === "16.0" || asversion === "16.1") {
                if (deleted.length || item.recurrenceInfo.getExceptionIds({}).length) {
                    TbSync.eventlog.add("info", syncData.eventLogInfo,
                        "EAS 16.1: Legacy <Exceptions> skipped – using InstanceId instead");
                }
            } else {
                // Old EAS 2.5 / 14.0 behaviour
                if (deleted.length || item.recurrenceInfo.getExceptionIds({}).length) {
